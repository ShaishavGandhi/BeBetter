...
public class LineView extends View {
    ...
    public void setColorArray(String[] colors){
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for(int k=0; k < drawDotLists.size(); k++){
        	...
        	for(Dot d: drawDotLists.get(k)){
        		if(showPopupType == SHOW_POPUPS_All && k%2==0)
        			drawPopup(canvas, String.valueOf(d.data), d.setupPoint(tmpPoint),Color.parseColor(colorArray[k%3]));
...
// 선택한 dot 만 popup 이 뜨게 한다.        
        if(showPopup && selectedDot != null){
            drawPopup(canvas,
                    ...
                    selectedDot.setupPoint(tmpPoint),R.drawable.popup_white);
        }
    }
